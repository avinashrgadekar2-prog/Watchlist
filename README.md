...
Click **Publish**. The `usernames` rule now also allows the *owner* of a username to update it (used when adding or changing a recovery email) — but never lets anyone reassign a username to a different account.
...
## How accounts work
Sign up with a username, email, and password. Your email is only used for password recovery — it's never shown to other members. You can sign in with **either** your email or your username, plus your password.

Forgot your password? Tap "Forgot password?" on the sign-in screen — it sends a real reset link to your email (Firebase's free built-in system, no cost, up to 50,000 users).

**Accounts made before this update:** these used a placeholder internal address instead of a real email, so "Forgot password" won't find anything for them yet. Go to Profile → tap the "Email" row → add a real email, and recovery will work from then on.
